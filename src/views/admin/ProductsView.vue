...
div
  ...
  v-row 
    v-col(cols="12")
      v-btn.mt-2(color="success" @click="openDialog(-1)") 新增商品
    v-col(cols="12")
      v-table
        thead
          tr
  ...
  v-dialog(v-model="form.dialog" persistent)
    v-form(v-model="form.valid" @submit.prevent="submit")
      v-card
        v-card-title 
        h2.text-center {{form._id.length > 0 ? '編輯商品' : '新增商品' }}
        v-card-text 
          v-row 
            v-col(cols="12")
              v-text-field(v-model="form.name" type="text" label="名稱" :rules="[rules.required]")
            v-col(cols="12")
              v-text-field(v-model="form.price" type="text" label="價格" :rules="[rules.required,rules.price]")
            v-col(cols="12")
              v-textarea(row="3" auto-grow v-model="form.description" label="說明" :rules="[rules.required]")
            v-col(cols="12")
